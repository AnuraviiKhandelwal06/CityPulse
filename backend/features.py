...
def build_feature_matrix(
    ...
    if not (data_path / "weather.csv").exists():
        alt_path = Path(__file__).parent / "data" / "raw"
        if (alt_path / "weather.csv").exists():
            data_path = alt_path

    weather = pd.read_csv(data_path / "weather.csv")
    traffic = pd.read_csv(data_path / "traffic.csv")
    incidents = pd.read_csv(data_path / "incidents.csv")
    zones = pd.read_csv(data_path / "zones.csv")
